...
class History:
    ...
    def save_fig(self, fig_name, display_lr=False):
        """
        Affiche deux graphiques pour la perte en entraînement et pour l'exactitude (accuracy) en
        entraînement. De plus, si le taux d'apprentissage a été sauvegardé, il est possible de lui
        afficher un graphique.
        ...
        axes[0].plot(epochs, self.history['acc'], label='Entraînement')
        axes[0].plot(epochs, self.history['val_acc'], label='Validation')
        axes[0].legend()

        axes[1].set_ylabel('Perte')
        axes[1].plot(epochs, self.history['loss'], label='Entraînement')
        axes[1].plot(epochs, self.history['val_loss'], label='Validation')
